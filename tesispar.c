...
void *Multiplicar_MatrizXT_por_MatrizX_threadOne(){

	int i, j, k;
  	float aux = 0;
  	for(k=0;k<33;k++){
		for(i=0;i<fila_invertida;i++){
			for(j=0;j<columna_invertida;j++){
...
void *Multiplicar_MatrizXT_por_MatrizX_threadTwo(){
...
void *Multiplicar_MatrizXT_por_MatrizX_threadthree(){

	int i, j, k;
  	float aux = 0;
  	for(k=66;k<fila_invertida;k++){
		for(i=0;i<fila_invertida;i++){
			for(j=0;j<columna_invertida;j++){
				aux = aux + (matriz_X_T[i][j]*matriz_X[j][k]);
			}
...
void Multiplicar_MatrizXT_por_MatrizY(double matriz_X_T[500][size], double matriz_Y[size][1], double matriz_XT_por_Y[200][1],int filas,int columnas){
...
void *Determinar_Matriz_Aux_threadOne(){
	int i, j, k;
	double aux = 0;
  	for(k=0;k<33;k++){
		for(i=0;i<filas;i++){
...
void *Determinar_Matriz_Aux_threadTwo(){
	int i, j, k;
	double aux = 0;
  	for(k=33;k<66;k++){
		for(i=0;i<filas;i++){
...
void *Determinar_Matriz_Aux_threadthree(){
	int i, j, k;
	double aux = 0;
  	for(k=66;k<columnas;k++){
		for(i=0;i<filas;i++){
        	for(j=0;j<columnas;j++){
				aux = aux + (matriz_X[i][j]*matriz_Inversa[j][k]);
				//printf("%.2f  %.2f \n", matriz_X_T[i][j], matriz_Y[j][0] );
...
//*************************************************************************************************************************************
//*************************************************************************************************************************************
void Tabla_Anova(double matriz_Y_circunfleja[size][1], double matriz_Y[size][1], double matriz_Inversa[500][500], double matriz_X[size][500], double matriz_XT_por_Y[200][1], double matriz_Aux[size][200], int filas, int columnas){
...
int main()
{
	...
	long int tiempoComputo;
  	double secs;
  	columna_invertida = filas;
  	//pthread_attr_t attribute;
   	void *exit_status;   
   	pthread_t *thread;
  	fila_invertida = columnas;
    ...
    pthread_create(&thread[0],NULL,Multiplicar_MatrizXT_por_MatrizX_threadOne,NULL);
    pthread_create(&thread[1],NULL,Multiplicar_MatrizXT_por_MatrizX_threadTwo,NULL);
    pthread_create(&thread[2],NULL,Multiplicar_MatrizXT_por_MatrizX_threadthree,NULL);
    int i;
    for (i = 0; i < 3; i = i + 1)
	...
	pthread_create(&thread[3],NULL,Determinar_Matriz_Aux_threadOne,NULL);
    pthread_create(&thread[4],NULL,Determinar_Matriz_Aux_threadTwo,NULL);
    pthread_create(&thread[5],NULL,Determinar_Matriz_Aux_threadthree,NULL);
	for (i = 3; i < 6; i = i + 1)
        pthread_join(thread[i],&exit_status);
